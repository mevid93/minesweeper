...
import miinaharava.gui.GamePanel;
import miinaharava.gui.SquareButton;
...
public class SquareButtonListener implements MouseListener {
	...
	private void playerLoseAction() {
		...
		minefield.paljastaKaikkiMiinat();
		markAsCleared();
	...
	private void clearSquareWithoutMine(int x, int y) {
		if (!gamegrid.getSquareFromCoordinate(x, y).isFlagged()) {
			...
			if (numOfCloseMines == 0) {
				if (x + 1 < gamegrid.getWidth() && !gamegrid.getSquareFromCoordinate(x + 1, y).isCleared()) {
				...
				if (y + 1 < gamegrid.getHeight() && !gamegrid.getSquareFromCoordinate(x, y + 1).isCleared()) {
				...
				if (x - 1 >= 0 && y - 1 >= 0 && !gamegrid.getSquareFromCoordinate(x - 1, y - 1).isCleared()) {
					clearSquareWithoutMine(x - 1, y - 1);
				}
